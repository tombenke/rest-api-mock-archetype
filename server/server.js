...
const restApi = require('{{rest_api_module_name}}');
const services = restApi.services;
const allServices = restApi.services.getServices();
...
server.configure(function() {
    server.use(proxy(config.serviceUrlPrefix, config.remoteServices, config.proxyException));
...
//    server.use('/services', express.static(services.servicesRoot));
//    console.log('servicesRoot: ', services.servicesRoot)
...
function writeHeader(response) {
    ...
    response.header('X-API-Version', restApi.version);
}
...
function registerServiceMethod(serviceDesc, method) {
    ...
    server[method.toLowerCase()](config.serviceUrlPrefix + reformatUrlPattern(serviceDesc.uriTemplate), function(request, response) {
